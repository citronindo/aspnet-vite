...
import { TemplateCustomizer } from "@apps/utils/template-customizer";
import { Menu } from "@apps/utils/menu";
import * as bootstrap from 'bootstrap';
import i18next from 'i18next';
import i18NextHttpBackend from 'i18next-http-backend';
import { config } from '../utils/config';

let isRtl = Helpers.isRtl(),
    // isDarkStyle = Helpers.isDarkStyle(),
    menu,
    animate,
...
(function(){
    ...
    layoutMenuEl.forEach(function (element) {
        ...
        showDropdownOnHover: localStorage.getItem('templateCustomizer-' + templateName + '--ShowDropdownOnHover') // If value(showDropdownOnHover) is set in local storage
            ...
            : templateCustomizer !== undefined // If value is set in config.js
            ? templateCustomizer.settings.defaultShowDropdownOnHover // Use the config.js value
  ...
  menuToggler.forEach(item => {
    item.addEventListener('click', event => {
      ...
      if (config.enableMenuLocalStorage && !Helpers.isSmallScreen()) {
        try {
    ...
    Helpers.swipeIn('.drag-target', function (e) {
        Helpers.setCollapsed(false);
    });

    // Detect swipe gesture on the target element and call swipe Out
    Helpers.swipeOut('#layout-menu', function (e) {
        if (Helpers.isSmallScreen()) Helpers.setCollapsed(true);
    });
  ...
  Helpers.initNavbarDropdownScrollbar();
  ...
  if (isHorizontalLayout || Helpers.isSmallScreen()) {
    return;
  }
