...
import customizerMarkup from './customizer/_template-customizer.html?raw';
// import CustomizerMarkup from './customizer/customizerMarkup';
// import { renderToString } from 'react-dom/server';
import { Helpers } from './helpers';

//const customizerMarkup = renderToString(CustomizerMarkup);
...
class TemplateCustomizer {
  constructor({
    ...
    if (this._ssr) return
    if (!Helpers) throw new Error('Helpers required.')
  ...
  setLayoutType(pos, updateStorage = true) {
    ...
    let menuScroll = Helpers.menuPsScroll
    ...
    if (this.settings.layoutType === 'fixed' || this.settings.layoutType === 'fixed-offcanvas') {
      ...
      if (PerfectScrollbarLib && menuScroll) {
        ...
        Helpers.menuPsScroll = menuScroll
      }
    } else if (menuScroll) {
  ...
  setDropdownOnHover(open, updateStorage = true) {
    ...
    if (Helpers.mainMenu) {
      Helpers.mainMenu.destroy()
      config.showDropdownOnHover = open

      const { Menu } = window

      Helpers.mainMenu = new Menu(document.getElementById('layout-menu'), {
