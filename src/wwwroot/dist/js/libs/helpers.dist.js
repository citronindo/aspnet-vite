...
const Helpers = {
  ...
  _scrollToActive(animate = false, duration = 500) {
    ...
    if (activeEl) {
      ...
      if (activeEl < parseInt(element.clientHeight * 2 / 3, 10))
        return;
  ...
  _findParent(el, cls) {
    if (el && el.tagName.toUpperCase() === "BODY" || el.tagName.toUpperCase() === "HTML")
      return null;
    el = el.parentNode;
  ...
  _supportsTransitionEnd() {
    ...
    TRANS_PROPERTIES.forEach((evnt) => {
      if (typeof el.style[evnt] !== "undefined")
        result = true;
    });
    return result;
  },
  ...
  _getAnimationDuration(el) {
    ...
    return parseFloat(duration) * (duration.indexOf("ms") !== -1 ? 1 : 1e3);
  },
  // ---
  ...
  _bindMenuMouseEvents() {
    ...
    if (!this._menuMouseEnter) {
      this._menuMouseEnter = () => {
        if (this.isSmallScreen() || !this._hasClass("layout-menu-collapsed") || this.isOffcanvas() || this._hasClass("layout-transitioning")) {
  ...
  off(event = requiredParam("event")) {
    ...
    let [, ...namespace] = event.split(".");
    namespace = namespace.join(".") || null;
    this._listeners.filter((listener) => listener.event === _event && listener.namespace === namespace).forEach((listener) => this._listeners.splice(this._listeners.indexOf(listener), 1));
  },
  // *******************************************************************************
  // * Life cycle
  init() {
  ...
  destroy() {
    ...
    this.off("init._Helpers");
    this._listeners.filter((listener) => listener.event !== "init").forEach((listener) => this._listeners.splice(this._listeners.indexOf(listener), 1));
  },
  // ---
